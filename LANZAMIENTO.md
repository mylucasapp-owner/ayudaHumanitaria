...
| Índices | Desplegados (feed, ráfagas, purga, denuncias) |
| Autenticación | Anónima y correo/contraseña habilitadas |
| Cloud Functions | 3 desplegadas en `us-east1` |
| App Check | Registrado y emitiendo tokens, **en monitoreo** |
| Pruebas | 46 casos, todos en verde |

## Antes de difundir la URL

Tres cosas que no puede hacer nadie más que tú.
...
### 3. Decidir sobre las teselas del mapa

Es el riesgo abierto más probable. El mapa apunta por defecto a
`tile.openstreetmap.org`, cuya política de uso no cubre picos masivos: si la
app se difunde de verdad, pueden bloquear el acceso y el mapa deja de cargar
para todos.

Mitigación ya incorporada: **la lista es la vista por defecto**, el mapa es
opt-in, y la app es plenamente usable sin él. Aun así, antes de una difusión
amplia conviene contratar un proveedor propio (MapTiler y Stadia Maps tienen
niveles gratuitos suficientes) y configurarlo:
...
2. **Teselas de OSM.** Ver arriba.
3. **Coordenadas públicas.** Saber qué casas están dañadas o vacías tiene valor
   para un ladrón. Difuminarlas rompería el producto; se asume y se compensa
   con verificación en terreno.
4. **Denuncias en manada.** No hay ocultamiento automático por número de
...
5. **Sin verificación de teléfono.** La defensa más fuerte contra cuentas
...
1. Verificación por SMS del oferente (punto 5 de arriba).
2. Proveedor de teselas propio.
3. Detección de duplicados al reportar: avisar "ya hay una necesidad parecida a
   200 m" antes de publicar. Baja el ruido que ahoga a los validadores.
4. Exportación para coordinación: CSV o vista imprimible por zona, porque en
   terreno se trabaja con papel cuando no hay señal.
5. Panel de zona para validadores, filtrando por radio en vez de ver todo.
